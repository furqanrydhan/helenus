...
/**
 * Adds the ability to get a slice of columns by its composite name
 * @param {Array} name_list The name of the composite columns to get
 * @returns {Object} a tuple of the column name, timestamp, ttl and value
 */
Row.prototype.get_slice = function(name_list){
    matches = {};
    for(key in this._map){
        split_key = key.split(":", 2);
        composite_key = split_key[0];
        if(name_list.indexOf(composite_key) > -1){
            if(!matches[composite_key]) matches[composite_key] = {};
            matches[composite_key][split_key[1]] = this[this._map[key]].value;
        }
    }
    return matches;
};
